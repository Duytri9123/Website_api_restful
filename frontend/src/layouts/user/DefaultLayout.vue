...
// // New: Reactive variable for mobile user dropdown
...
// // New: Reference for the mobile user dropdown toggle button
...
//   isLoggedIn.value = localStorage.getItem("authToken") ? true : false;
// };
...
// // New: Function to toggle the mobile user dropdown
...
//   ) {
//     mobileMenuOpen.value = false;
//     // When mobile menu closes, also close the mobile user dropdown
//     mobileUserDropdownOpen.value = false;
//   }

//   // New: Handle click outside for mobile user dropdown
//   if (
...
//   ) {
//     // This condition needs refinement if the dropdown can be outside the menu content directly
//     // For now, it assumes the dropdown content is always within mobileMenuContentRef
//   }
...
<template>
  <header
    class="bg-white border-b border-gray-200 dark:bg-gray-900 dark:border-gray-700 shadow-sm"
  >
    <nav
      ...
      >
        ...
        >
          ...
          >
            ...
            <span
              v-if="hasNotifications"
              class="absolute top-0 right-0 inline-flex items-center justify-center px-2 py-1 text-xs font-bold leading-none text-blue-100 bg-blue-600 rounded-full"
              >{{ notificationCount }}</span
            >
          </button>
          <!-- Favorites -->
