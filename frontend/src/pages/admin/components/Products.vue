...
// Get average rating for a product (assuming it comes from API)
const getAverageRating = (product) => {
  // If product has average_rating field from API
...
const getLeafCategories = computed(() =>
  categories.value.filter((cat) => !categories.value.some((c) => c.parent_id === cat.id))
);

// kiem tra gia sale khong lon hon gia ban
const validateSellingPrice = () => {
  ...
  if (isNaN(originalPrice) || originalPrice <= 0) {
    sellingPriceError.value = "Please enter the original price first.";
    return;
  }
  if (!isNaN(sellingPrice) && sellingPrice > originalPrice) {
    sellingPriceError.value = "Sale price cannot be greater than the original price.";
  }
};

//cap nhat form gia khi nhap
watch([() => form.original_price, () => form.selling_price], validateSellingPrice);

// Hàm xử lý khi chọn hoặc kéo thả file
...
const AddProduct = async () => {
  try {
    ...
    const response = await axiosAdmin.post(`/api/products`, formData, {
      headers: {
    ...
    error.value = "Failed to save product. Please check input fields.";
  }
};
...
const closeEditProduct = async (product) => {
  isEditModal.value = false;
};
const UpdateProduct = async (product) => {
  try {
    ...
    if (form.deleted_images.length > 0) {
      form.deleted_images.forEach((idImage) => {
        formData.append("deleted_images[]", parseInt(idImage));
      });
    }

    const response = await axiosAdmin.post(`/api/products/${form.id}`, formData, {
      headers: {
        "Content-Type": "multipart/form-data",
    ...
    error.value = "Failed to update product. Please try again later.";
  ...
  <div>
            ...
            <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
              ...
              <tbody>
                ...
                >
                  ...
                  <td class="px-4 py-3">{{ product.original_price }}</td>
                  <td
                    class="px-4 py-3 font-medium text-gray-900 whitespace-nowrap dark:text-white"
                  >
                    {{ product.status }}
        ...
        >
          ...
          <form @submit.prevent="AddProduct">
            <div class="grid gap-4 mb-4 sm:grid-cols-2">
              ...
              <div class="grid gap-4 sm:col-span-1 md:gap-2 sm:grid-cols-3">
                <div>
                  <label
                    for="weight"
                    class="block mb-2 text-sm font-medium text-gray-900 dark:text-white"
              ...
              <!-- description -->
              <div class="grid gap-4 sm:col-span-2 md:gap-2 sm:grid-cols-3">
            ...
            <div class="mb-4 p-4">
              ...
              <div
                class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 lg:grid-cols-4 gap-4 mb-6"
              >
                <div
                  v-for="(image, index) in productImages"
                  :key="index"
                  class="relative bg-white rounded-lg shadow-md p-2 flex items-center justify-center border"
                  :class="{ 'border-orange-400 border-2': image.highlighted }"
                >
                  <img
                    :src="image.url"
                    alt="image"
                  ...
                  <button
                    ...
                    :class="{ 'bg-red-600': image.highlighted }"
                    title="Remove image"
                  >
                    <svg
                      class="w-4 h-4"
    ...
    >
      <form
        ...
        :class="[
          'fixed top-16 left-0 z-40 w-full h-[calc(100%-4rem)] max-w-3xl p-4 overflow-y-auto transition-transform bg-white dark:bg-gray-800',
          { 'translate-x-0': isEditModal, '-translate-x-full': !isEditModal },
        ]"
        tabindex="-1"
      >
        <h5
        ...
        <button
          type="button"
          @click.prevent="closeEditProduct"
          ...
          <div class="space-y-4 sm:col-span-2 sm:space-y-6">
            ...
            <div class="mb-4">
              ...
              <div class="flex items-center justify-center w-full">
                <label
                  for="dropzone-file"
                  class="flex flex-col items-center justify-center w-full h-35 border-2 border-gray-300 border-dashed rounded-lg cursor-pointer bg-gray-50 dark:hover:bg-bray-800 dark:bg-gray-700 hover:bg-gray-100 dark:border-gray-600 dark:hover:border-gray-500 dark:hover:bg-gray-600"
                >
